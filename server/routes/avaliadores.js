...
import { createPasswordHash, validatePassword } from '../lib/password.js';
...
avaliadoresRouter.post('/', (req, res) => {
  ...
  try {
    const result = db.prepare("INSERT INTO avaliadores (nome, email, username, senha_hash, senha_salt, papel) VALUES (?, ?, ?, ?, ?, 'avaliador')").run(nome, email, username, credentials.hash, credentials.salt);
    res.status(201).json(db.prepare('SELECT id, nome, email, username, ativo, criado_em FROM avaliadores WHERE id = ?').get(result.lastInsertRowid));
  } catch (error) {
...
avaliadoresRouter.put('/:id', (req, res) => {
  ...
  try {
    const result = password
      ? (() => { const credentials = createPasswordHash(password); return db.prepare('UPDATE avaliadores SET nome = ?, email = ?, username = ?, senha_hash = ?, senha_salt = ? WHERE id = ? AND papel = ?').run(nome, email, username, credentials.hash, credentials.salt, id, 'avaliador'); })()
      : db.prepare('UPDATE avaliadores SET nome = ?, email = ?, username = ? WHERE id = ? AND papel = ?').run(nome, email, username, id, 'avaliador');
