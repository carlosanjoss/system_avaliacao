...
function normalizeAssignment(tipo, ids) {
  if (!['individual', 'dupla'].includes(tipo)) throw new HttpError(400, 'Tipo de avaliação inválido.');
  const unique = [...new Set((ids || []).map(Number))];
  const expected = tipo === 'dupla' ? 2 : 1;
  if (unique.length !== expected || unique.some((id) => !Number.isInteger(id) || id <= 0)) {
    throw new HttpError(400, `Selecione ${expected} avaliador${expected > 1 ? 'es' : ''} para o modo ${tipo}.`);
  }
  ...
  if (found !== expected) throw new HttpError(400, 'Um ou mais avaliadores não possuem usuário e senha ativos. Edite o cadastro antes de atribuir o lote.');
  return unique;
}
...
function refreshStatus(loteId) {
  ...
  `).get(loteId);
  if (!stats) return;
  const required = stats.itens * stats.avaliadores;
  let status = 'pendente';
...
lotesRouter.get('/', (req, res) => {
  ...
  res.json(rows.map((row) => ({ ...row, progresso: row.total_itens ? row.avaliacoes_feitas / (row.total_itens * row.total_avaliadores) : 0 })));
});
...
lotesRouter.post('/', requireAdmin, (req, res) => {
  ...
  const tipo = req.body.tipoAvaliacao;
  const avaliadores = normalizeAssignment(tipo, req.body.avaliadoresAtribuidos);
  ...
  const loteId = transaction(() => {
    const lote = db.prepare(`
      INSERT INTO lotes (nome_arquivo, coluna_conteudo, coluna_resultado, tipo_avaliacao)
      VALUES (?, ?, ?, ?)
    `).run(nomeArquivo, colunaConteudo, colunaResultado, tipo);
    ...
    avaliadores.forEach((avaliadorId, index) => assign.run(id, avaliadorId, index + 1));
    return id;
  });

  res.status(201).json(db.prepare(`${loteSelect} WHERE l.id = ? GROUP BY l.id`).get(loteId));
});
...
lotesRouter.get('/:id/pendentes/:avaliadorId', requireEvaluator, (req, res) => {
  ...
  const items = db.prepare(`
    ...
    WHERE li.lote_id = ? ${condition}
    GROUP BY li.id ORDER BY li.linha_index LIMIT ? OFFSET ?
  `).all(avaliadorId, loteId, limit, (page - 1) * limit).map((row) => ({ ...row, categorias: JSON.parse(row.categorias) }));
  const stats = db.prepare(`
...
lotesRouter.patch('/:id', requireAdmin, (req, res) => {
  ...
  const tipo = req.body.tipoAvaliacao || lote.tipo_avaliacao;
  ...
  const avaliadores = normalizeAssignment(tipo, assigned);
  ...
  if (hasReviews && (tipo !== lote.tipo_avaliacao || req.body.avaliadoresAtribuidos)) throw new HttpError(409, 'Não é possível alterar o tipo ou a atribuição após o início das avaliações.');
  transaction(() => {
    db.prepare('UPDATE lotes SET tipo_avaliacao = ? WHERE id = ?').run(tipo, id);
    if (!hasReviews) {
  ...
  refreshStatus(id);
  res.json(db.prepare(`${loteSelect} WHERE l.id = ? GROUP BY l.id`).get(id));
});
